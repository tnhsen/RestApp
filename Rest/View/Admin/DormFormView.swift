        ...
        NavigationView{
            GeometryReader { geometry in
                ZStack{
                    ...
                    Text("ลงทะเบียนหอพัก").font(.title).position(x: geometry.size.width / 2, y: geometry.size.height / 15)
...
struct DormForm_Previews: PreviewProvider {
    static var previews: some View {
        DormFormView().environmentObject(DataModel())
    }
}
